...
/** Searches classpath for @Traversal|@TraversalSource and @Doc annotations (via reflection). Used
  * for `.help` step. There are two use cases for this, which require slightly different
  * implementations 1) `myDomain.help` - for the node starter steps 2) `myDomain.someNodeType.help`
  * \- for steps that are available a specific node type
  *
  * For use case 2, we also take into account all parent traits of a node type, recursively. I.e. if
  * `SomeNodeType` has a base type `SomeBaseType`, and there are steps defined for
  * `Traversal[SomeBaseType]`, we will include those in the results.
  *
    ...
    def forElementSpecificSteps(elementClass: Class[?], verbose: Boolean): String =
        ...
        val stepDocs =
            ...
            if !verbose then elementSpecificDocs
                ...
                elementSpecificDocs ++ genericStepDocs ++ genericNodeStepDocs
            else elementSpecificDocs ++ genericStepDocs

        val table = Table(
          columnNames = if verbose then ColumnNamesVerbose else ColumnNames,
          rows = stepDocs.sortBy(_.methodName).map { stepDoc =>
              val baseColumns = List(s".${stepDoc.methodName}", stepDoc.doc.info)
              if verbose then baseColumns :+ stepDoc.traversalClassName
              else baseColumns
          }
        )
