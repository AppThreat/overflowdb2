...
class TraversalSugarExt[A](val iter: Iterator[A]) extends AnyVal:
    ...
    def groupMapReduce[K, B](key: A => K)(f: A => B)(reduce: (B, B) => B): Map[K, B] =
        l.groupMapReduce(key)(f)(reduce)

    /** Execute the traversal and return a mutable.Set (better performance than `immutableSet` and
      * has stable iterator order)
      */
    ...
    def last: A =
        iter.hasNext
        var res = iter.next()
        while iter.hasNext do res = iter.next()
        res

    def lastOption: Option[A] =
        if iter.hasNext then Some(last) else None
    ...
    /** Print help/documentation based on the current elementType `A`. Relies on all step extensions
      * being annotated with \@Traversal / @Doc Note that this works independently of tab completion
      * and implicit conversions in scope - it will simply list all documented steps in the
      * classpath
      */
    ...
    def help[B >: A](implicit elementType: ClassTag[B], searchPackages: DocSearchPackages): String =
        new TraversalHelp(searchPackages).forElementSpecificSteps(
...
class TraversalLogicExt[A](val iterator: Iterator[A]) extends AnyVal:
    ...
    /** perform side effect without changing the contents of the traversal will only apply the
      * partialFunction if it is defined for the given input - analogous to `collect`
      */
    @Doc(info = "perform side effect without changing the contents of the traversal")
    def sideEffectPF(pf: PartialFunction[A, ?]): Traversal[A] =
        sideEffect(pf.lift)
    ...
    def where(trav: Traversal[A] => Traversal[?]): Traversal[A] =
        iterator.filter { (a: A) =>
    ...
    def whereNot(trav: Traversal[A] => Traversal[?]): Traversal[A] =
        iterator.filter { (a: A) =>
            !trav(Iterator.single(a)).hasNext
        }

    /** only preserves elements if the provided traversal does _not_ have any results - alias for
      * whereNot
      */
    @Doc(info =
        "only preserves elements if the provided traversal does _not_ have any results - alias for whereNot"
    )
    def not(trav: Traversal[A] => Traversal[?]): Traversal[A] =
        whereNot(trav)

    /** only preserves elements for which _at least one of_ the given traversals has at least one
      * result Works for arbitrary amount of 'OR' traversals.
      *
      * @example
    ...
    def or(traversals: (Traversal[A] => Traversal[?])*): Traversal[A] =
        iterator.filter { (a: A) =>
    ...
    def and(traversals: (Traversal[A] => Traversal[?])*): Traversal[A] =
        iterator.filter { (a: A) =>
    ...
    /** Branch step: based on the current element, match on something given a traversal, and provide
      * resulting traversals based on the matched element. Allows to implement conditional
      * semantics: if, if/else, if/elseif, if/elseif/else, ...
      *
      * @param on
      *   Traversal to get to what you want to match on
      * @tparam BranchOn
      *   required to be >: Null because the implementation is using `null` as the default value. I
      *   didn't find a better way to implement all semantics with the niceties of PartialFunction,
      *   and also yolo...
      * @param options
      ...
      */
    @Doc(info =
        "allows to implement conditional semantics: if, if/else, if/elseif, if/elseif/else, ..."
    )
...
class TraversalTrackingExt[A](val iterator: Iterator[A]) extends AnyVal:
    ...
    /** retrieve entire path that has been traversed thus far prerequisite: enablePathTracking has
      ...
      *   }}}
      *   TODO would be nice to preserve the types of the elements, at least if they have a common
      *   supertype
      */
...
class TraversalRepeatExt[A](val trav: Iterator[A]) extends AnyVal:
    ...
    /** Repeat the given traversal
      *
      * By default it will continue repeating until there's no more results, not emit anything along
      * the way, and use depth first search.
      *
      * The @param behaviourBuilder allows you to configure end conditions (until|whilst|maxDepth),
      * whether it should emit elements it passes by, and which search algorithm to use (depth-first
      * or breadth-first).
      *
      * Search algorithm: Depth First Search (DFS) vs Breadth First Search (BFS): DFS means the
      * repeat step will go deep before wide. BFS does the opposite: wide before deep. For example,
      * given the graph {{{L3 <- L2 <- L1 <- Center -> R1 -> R2 -> R3 -> R4}}} DFS will iterate the
      * nodes in the order: {{{Center, L1, L2, L3, R1, R2, R3, R4}}} BFS will iterate the nodes in
      * the order: {{{Center, L1, R1, R1, R2, L3, R3, R4}}}
      *
      * @example
      *   {{{
      ...
      *   }}}
      * @note
      *   this works for domain-specific steps as well as generic graph steps - for details please
      *   take a look at the examples in RepeatTraversalTests: both '''.followedBy''' and '''.out'''
      *   work.
      * @see
    ...
    def repeat[B >: A](
      ...
      behaviourBuilder: RepeatBehaviour.Builder[B] => RepeatBehaviour.Builder[B] =
        ...
        val _repeatTraversal =
            repeatTraversal
                .asInstanceOf[Traversal[B] => Traversal[
                  B
