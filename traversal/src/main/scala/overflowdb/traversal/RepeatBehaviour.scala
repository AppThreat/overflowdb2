...
object RepeatBehaviour:
    ...
    class Builder[A]:
        ...
        /** Emit intermediate elements (along the way), if they meet the given condition. Note that
          * this does not apply a filter on the final elements of the traversal.
          */
        ...
        /** Stop traversing when given condition-traversal has no result. The condition-traversal is
          * already evaluated at the first iteration, for classic while/repeat behaviour.
          *
          * n.b. the only reason not to call this `while` is to avoid using scala keywords, which
        ...
        /** Maximum depth to go down in the repeat traversal. Note that there may be other
          * conditions like until|whilst etc.
          */
        @deprecated(
        ...
        private[traversal] def build: RepeatBehaviour[A] =
            new RepeatBehaviour[A]:
                ...
                override def shouldEmit(element: A, currentDepth: Int): Boolean =
                    _shouldEmit(element, currentDepth)
    end Builder
end RepeatBehaviour
