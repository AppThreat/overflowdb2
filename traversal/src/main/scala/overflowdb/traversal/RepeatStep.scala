...
object RepeatStep:
    ...
    def apply[A](
      ...
      behaviour: RepeatBehaviour[A]
        ...
        (element: A) =>
            new RepeatStepIterator[A](
              element,
              elem => repeatTraversal(Iterator.single(elem)),
              behaviour
            )

            /** stores work still to do. depending on the underlying collection type, the behaviour
              * of the repeat step changes
              */
    trait Worklist[A]:
...
class RepeatStepIterator[A](
  ...
  behaviour: RepeatBehaviour[A]
        ...
        while worklist.nonEmpty && !stop do
            ...
            if trav.isEmpty then worklist.removeHead()
                ...
                then
                    ...
                    if behaviour.shouldEmit(element, depth) then emitSack.enqueue(element)
                    if emitSack.nonEmpty then stop = true
            end if
        end while
    ...
    override def next(): A =
        ...
        if behaviour.dedupEnabled then visited.addOne(result)
        result
end RepeatStepIterator
