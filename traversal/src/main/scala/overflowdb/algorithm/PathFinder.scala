...
object PathFinder:
    def apply(nodeA: Node, nodeB: Node, maxDepth: Int = -1): Seq[Path] =
        if nodeA == nodeB then Seq(Path(Seq(nodeA)))
        else
            Iterator
                .single(nodeA)
                .enablePathTracking
                .repeat(_.both) { initialBehaviour =>
                    val behaviour =
                        initialBehaviour.dedup // no cycles
                            .until(
                              _.is(nodeB)
                            ) // don't continue on a given path if we've reached our destination
                    if maxDepth > -1 then behaviour.maxDepth(maxDepth)
                    else behaviour
                }
    ...
    private def edgesBetween(nodeA: Node, nodeB: Node): Seq[EdgeEntry] =
        val outEdges = nodeA.outE.asScala.filter(_.inNode == nodeB).map(edge =>
            EdgeEntry(Direction.OUT, edge.label)
        )
        val inEdges = nodeA.inE.asScala.filter(_.outNode == nodeB).map(edge =>
            EdgeEntry(Direction.IN, edge.label)
        )
