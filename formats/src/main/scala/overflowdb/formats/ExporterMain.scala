...
/** Base functionality to export a given OverflowDB graph to various export formats. Because ODB
  * relies on domain specific implementations, specifically the NodeFactories and EdgeFactories from
  * the domain-specific generated classes (typically generated by by
      ...
      edgeFactories: Seq[EdgeFactory[?]]
        ...
        OParser
            ...
            .map { case Config(inputFile, format, outputFile) =>
                if Files.notExists(inputFile) then
                    throw new AssertionError(s"given input file $inputFile does not exist")
                if Files.exists(outputFile) then
                    if Files.isRegularFile(outputFile) then
                        throw new AssertionError(
                          s"output file $outputFile already exists and is not a directory"
                        )
                else
