...
object Neo4jCsvExporter extends Exporter:
    ...
    /** Exports OverflowDB Graph to neo4j csv files see
      ...
      *
      * For both nodes and relationships, we first write the data file and to derive the property
      * types from their runtime types. We will write columns for all declared properties, because
      * we only know which ones are actually in use *after* traversing all elements.
      */
      ...
      outputRootDirectory: Path
        ...
        Using.resource(CSVWriter.open(dataFile.toFile, append = false)) { writer =>
            nodes.iterator.foreach { node =>
                if columnDefinitions == null then
                    columnDefinitions = new ColumnDefinitions(node.propertyKeys.asScala)

                val specialColumns = Seq(node.id.toString, node.label)
                val propertyValueColumns =
                    columnDefinitions.propertyValues(node.propertyOption(_).toScala)
        ...
        val cypherPropertyMappings =
            columnDefinitions.propertiesMappingsForCypher(startIndex = 2).mkString(",\n")
        val cypherQuery =
            ...
            case EdgeFilesContext(
                ...
                writeSingleLineCsv(
                  ...
                  Seq(
                    ...
                    ColumnType.EndId,
                    ColumnType.Type
                  ) ++ columnDefinitions.propertiesWithTypes
                )
                ...
                val cypherPropertyMappings =
                    columnDefinitions.propertiesMappingsForCypher(startIndex = 3).mkString(",\n")
                val cypherQuery =
