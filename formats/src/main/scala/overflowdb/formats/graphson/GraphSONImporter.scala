...
object GraphSONImporter extends Importer:
    ...
    private def addNode(n: Vertex, graph: Graph): Unit =
        graph.addNode(n.id.`@value`, n.label, flattenProperties(n.properties, graph)*)

    private def flattenProperties(m: Map[String, Property], graph: Graph): Array[?] =
        m.view
            .mapValues { v =>
                v.`@value` match
    ...
    private def addEdge(e: Edge, graph: Graph): Unit =
        ...
        src.addEdge(e.label, tgt, flattenProperties(e.properties, graph)*)
end GraphSONImporter
