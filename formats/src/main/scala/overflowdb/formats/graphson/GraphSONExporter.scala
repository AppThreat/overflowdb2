...
object GraphSONExporter extends Exporter:
      ...
      propertyType: String
        ...
        element.propertiesMap.asScala.map { case (propertyName, propertyValue) =>
            propertyName -> Property(
              LongValue(propertyId.getAndIncrement()),
    ...
    def valueEntry(propertyValue: Any): PropertyValue =
        ...
        propertyValue match
            case x: Array[?]              => ListValue(x.map(valueEntry))
            case x: Iterable[?]           => ListValue(x.map(valueEntry).toArray)
            case x: IterableOnce[?]       => ListValue(x.iterator.map(valueEntry).toArray)
            case x: java.lang.Iterable[?] => ListValue(x.asScala.map(valueEntry).toArray)
